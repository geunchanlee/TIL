...
1. 어제 내용 복습 
    ...
    자바스크립트 코드는 어디에 위치해야 할까?

    - DOM 구성이 완성되기 전에 동작하는 걸 방지하기 위해 안전하게 자바스크립트 코드는 하단에 넣는다.

    

2. jQuery DOM Traversing

    DOM 탐색을 여행 사이트 예제를 통해서 실습하고 메서드들 살펴봄

    - 클릭 이벤트, 키보드 이벤트, 포커스 이벤트 각 경우에 대해 예제로 학습

        - 각 이벤트에서 사용할 수 있는 메소드들

    - DOM Traversing의 예시와 만날 수 있는 특정 상황들에 대해서 학습

        - 기존 요소와 새로 들어갈 요소의 위치를 어떻게 할 지..

    - `$(this).parents`, `(this).closest` 메소드

    - HTML5의 `data-key='value'` 속성

    - 문자열 숫자로 형변환

    - 이벤트를 처리할 때 사용자에게 입력을 어떻게 받을 지. 꼭 모든 경우를 열어둬야 하는지 고민할 요소
